...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, stop_train_transient, eval_path):
    ...
    flow_path = Path(dataset.model_path) / "flow_dynamic"
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if viewpoint_cam.flow is not None:
            ...
            with torch.no_grad():
                if iteration % 100 == 0:
                    large_flow_img = gt_image.clone()  # [3, h, w]
                    large_flow_img[1] = large_flow_img[1] + large_flow
                    large_flow_img = Image.fromarray((large_flow_img.detach().permute(1, 2, 0).cpu().numpy() * 255).astype('uint8'))
                    large_flow_img.save(str(flow_path / f"{iteration}.png"))

            flow_loss = torch.abs(large_flow * (weights-large_flow)).mean()
            flow_weight = 0.5 * (iteration / opt.iterations)
